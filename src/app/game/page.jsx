...
export default function Game() {
  ...
  return (
    <div className="flex flex-col justify-center items-center h-screen">
      ...
      <div
        className={`${styles.gameInfo} flex absolute top-[120px] md:top-[200px] w-full justify-around text-white p-5 mb-5 z-10 `}
      >
        <div className="timer">Time left: {timer} seconds</div>
        <div className="score">Score: {score}</div>
        <div className="lives">Lives: {lives}</div>
      </div>
      <div className={`grid pt-16 md:pt-36 gap-2.5 ${styles[level]}`}>
        {cards.length === 0 ? (
          <p>Loading...</p>
        ) : (
          cards.map((card) => (
